...
public class DiningHakkers {
   ...
   private Behavior<NotUsed> behavior() {
      ...
      //Create 5 chopsticks
      List<ActorRef<Chopstick.Command>> chopsticks = IntStream.range(0, 5)
      ...
      IntStream.range(0, names.size())
         .mapToObj(i -> {
            Behavior<Hakker.Command> hakker = Hakker.create(names.get(i), chopsticks.get(i), chopsticks.get((i + 1) % 5));
