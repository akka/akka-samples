...
object ClusterClientSpec extends MultiNodeConfig {
  ...
  nodeConfig(first)(ConfigFactory.parseString(s"""
    akka.cluster.client.grpc.receptionist.canonical.port = ${grpcPorts(first)}
    """))

  nodeConfig(second)(ConfigFactory.parseString(s"""
    akka.cluster.client.grpc.receptionist.canonical.port = ${grpcPorts(second)}
    """))

  nodeConfig(third)(ConfigFactory.parseString(s"""
    akka.cluster.client.grpc.receptionist.canonical.port = ${grpcPorts(third)}
    """))

  nodeConfig(fourth)(ConfigFactory.parseString(s"""
    akka.cluster.client.grpc.receptionist.canonical.port = ${grpcPorts(fourth)}
    """))

  testTransport(on = true)

  case class Reply(msg: Any, node: HostPort)
...
class ClusterClientSpec
  ...
  def roleName(addr: HostPort): Option[RoleName] = {
    grpcPorts
      .collect {
        case (role, port) if remainingServerRoleNames(role) => (port, role)
      }
      .toMap
      .get(addr.port)
  }
