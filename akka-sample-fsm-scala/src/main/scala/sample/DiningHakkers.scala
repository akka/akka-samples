...
*/
class Hakker(ctx: ActorContext[HakkerMessage], name: String, left: ActorRef[ChopstickMessage], right: ActorRef[ChopstickMessage]) {
  ...
  private lazy val thinking: Behavior[HakkerMessage] = {
    Behaviors.receiveMessage {
      case Eat =>
        left ! Take(adapter)
        right ! Take(adapter)
  ...
  private def waitForOtherChopstick(chopstickToWaitFor: ActorRef[ChopstickMessage],
...
object DiningHakkers {
  ...
  def mainBehavior: Behavior[NotUsed] = Behaviors.setup { context =>
    ...
    } yield context.spawn(Behaviors.setup[HakkerMessage] { ctx => new Hakker(ctx, name, chopsticks(i), chopsticks((i + 1) % 5) ).waiting }, name)
