...
object ShoppingCart {
    ...
    */
    case class UpdateItem(productId: String, quantity: Int, replyTo: ActorRef[Result]) extends Command with ExpectingReply[Result]
    ...
    */
    ...
    def openShoppingCart(state: State, command: Command): Effect[Event, State] = command match {
        ...
        case cmd @ UpdateItem(productId, quantity, _) =>
            Effect.persist(ItemUpdated(productId, quantity))
                .thenReply(cmd)(_ => OK)
